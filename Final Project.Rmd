...
and the game effect. Thus, the odds that somebody in the trust game would give 5 dollars
is 2.32 (95 confidence interval 1.17,4.17) higher than somebody in the lottery game.

# Bayes Analysis Model 2 - Time to Decide

```{r}
#HIstograms Raw
hist(trust$lnt.decision.page.submit)
hist(trust$lt.decision.page.submit)
hist(trust$tnt.decision.page.submit)
hist(trust$tt.decision.page.submit)

# Data summary for DV ~ conditions
datasummary(submit.decision.sec ~ condition * (mean + sd), data = trustlong)
```
...
hist(log(trust$lnt.decision.page.submit))
hist(log(trust$lt.decision.page.submit))
hist(log(trust$tnt.decision.page.submit))
hist(log(trust$tt.decision.page.submit))

# Data summary for DV ~ conditions
datasummary(log(submit.decision.sec) ~ condition * (mean + sd), data = trustlong)
```


Model Equation:

$$Time_i = Lognormal(\mu ,\sigma)$$
